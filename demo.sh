#!/usr/bin/env bash
# ============================================================================
# demo.sh -- ONE combined, guided, paused presentation of the A64 maze runner.
# It merges the behaviour demo AND the assembly deep dive, split across 3
# speakers and BALANCED BY TIME (slides differ in length; the slow solve alone
# is ~2 min, so speakers get a different number of slides each).
#
#   Speaker 1  "It's bytes and bits"   ~3:00   (3 slides)  Chapter 4
#   Speaker 2  "Watch it think"        ~3:25   (3 slides)  the search
#   Speaker 3  "Down to the metal"     ~3:30   (4 slides)  Chapter 9 / AAPCS64
#   Total ~10 min.  (To hit 8: skip the slides marked [trim].)
#
# Run with:  make demo      (leave RUN empty on the Pi)
# Off-Pi:    RUN="qemu-aarch64 -L /usr/aarch64-linux-gnu" \
#            CC=aarch64-linux-gnu-gcc OBJDUMP=aarch64-linux-gnu-objdump \
#            GDB=gdb-multiarch ./demo.sh
# ============================================================================
set -u
CC="${CC:-gcc}"
...
speaker_card() {   # $1=number  $2=title  $3=time  $4=slides-summary
  clear
  echo
  ...
  printf '%s\n' "$4"
  echo
  printf '\033[1;93m  >> Speaker %s: press Enter when you are ready...\033[0m' "$1"
  read -r _
}

# Slide header with its budgeted time.
slide() {          # $1="S1 1/3"  $2=title  $3=~time
  clear
...
slide "S1 2/3" "Inside ONE byte: walls + state, as bits" "55s"
...
slide "S1 3/3" "Signed deltas: two's complement & sign extension" "50s"
...
# ====================================================== SPEAKER 2 ===========
speaker_card "2" "Watch it think  (the search)" "~3:25, 3 slides" \
"   1) the SLOW animated solve (the centrepiece)
   2) an unsolvable maze -> exit code 1
   3) prove it is correct against a C reference"

slide "S2 1/3" "The recursive solve, SLOW, with the live call stack" "2:00"
cat <<EOF
  Narrate the HUD while it runs:
...
echo
echo "  Explored, backtracked, highlighted the path.  Exit code: $?"
pause

slide "S2 2/3" "An UNSOLVABLE maze -- goal walled off" "50s"
...
slide "S2 3/3" "Is it CORRECT? diff A64 vs a C reference" "35s"
...
# ====================================================== SPEAKER 3 ===========
speaker_card "3" "Down to the metal  (Chapter 9 / AAPCS64)" "~3:30, 4 slides" \
...
slide "S3 1/4" "AAPCS64 register roles (the contract)" "45s"
cat <<'EOF'
...
slide "S3 2/4" "A whole routine as real machine code: move()" "45s"
...
slide "S3 3/4" "The calling convention inside solve()" "50s"
...
slide "S3 4/4" "LIVE: watch the real stack frames stack up" "55s"
